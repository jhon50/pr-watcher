...
5. **Missing-test-FILE pass (run this FIRST, before the coverage map).** This is a per-FILE check, not a per-line one, and it is easy to skip — do it deliberately. List every non-trivial source file the diff ADDS or gives new public behavior. For EACH one, resolve its expected test path using the source→test mapping in the REVIEW RULES above, then check whether that specific test file is present in this PR's diff (for a new source file) or exists and is touched (for changed behavior). For every source file whose expected test file is absent, emit a SEPARATE `important` finding (confidence `high`), pinned to the `+` line of that source file, naming the exact missing test path. Hard constraints:
   - **The presence of ANY test file in the change does NOT satisfy the others.** A multi-file unit (e.g. an EntryPoint + Action + Form + Query) is only covered when EACH present piece has its own test. Seeing one `*_test.rb` (commonly the EntryPoint test) is NOT a reason to consider the Form/Action/Query tested — check each independently.
   - **One finding per missing test file.** Do not collapse "Form, Action and Query are all untested" into a single finding — the author needs one actionable line per missing file.
   - Excluded: pure refactors with no behavior change, generated files, schema/migration dumps, config, docs, and one-off/data-migration scripts.
